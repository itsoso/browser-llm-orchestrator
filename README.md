...
基于 Chatlog 集成，我们进一步提供了**每日复盘功能**，支持批量处理多个群聊的每日总结，并提供 Web 可视化管理界面和公开分享 API。

### 核心特性

| 特性 | 说明 |
|:---|:---|
| **批量处理** | 一次选择多个群聊，自动生成所有群聊的每日总结 |
| **进度跟踪** | 实时查看每个群聊的处理状态和进度 |
| **历史管理** | 保存所有批次的历史记录，方便回溯查看 |
| **灵活配置** | 支持选择不同的 LLM 和模型版本 |
| **公开分享** | 提供 API 接口，支持小程序等外部应用访问 |
| **智能优化** | 自动处理长文本，避免输入截断 |

### 使用场景

1. **个人知识管理**：每天自动复盘所有群聊，生成结构化笔记
2. **团队协作**：每周复盘项目群聊，公开分享给团队成员
3. **内容创作**：从群聊中提取有价值的讨论和灵感
...
- **批次数据**：`data/daily_recaps/batch_{batch_id}.json`
- **复盘结果**：存储在 Obsidian Vault 中（与 chatlog_automation 相同）

### 详细文档

完整使用指南请参阅：[DAILY_RECAP_USAGE.md](./DAILY_RECAP_USAGE.md)
